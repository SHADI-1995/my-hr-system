...
@section('content')
    <style>
        ...
        .field select,.field textarea{width:100%;border:1px solid #ddd6fe;border-radius:14px;padding:10px 12px;font-size:12px;font-weight:800;outline:none;background:#fff}
        .field select{height:44px}.field textarea{min-height:80px;resize:vertical}
        .actions{display:flex;gap:8px;flex-wrap:wrap}
        .btn{height:38px;border:0;border-radius:12px;padding:0 13px;font-size:11px;font-weight:900;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;gap:7px;cursor:pointer}
        .primary{background:#6d5bd0;color:#fff}.soft{background:#ede9fe;color:#4c3b91}.green{background:#16a34a;color:#fff}.orange{background:#f59e0b;color:#fff}.red{background:#dc2626;color:#fff}.dark{background:#111827;color:#fff}
        ...
        td{border-top:1px solid #f1eefb;padding:9px 5px;font-size:10px;font-weight:800;text-align:center;word-break:break-word;vertical-align:middle}
        .pill{display:inline-flex;padding:5px 9px;border-radius:999px;font-size:10px;font-weight:900;white-space:nowrap}
        .generated{background:#dbeafe;color:#1d4ed8}.sent{background:#fef3c7;color:#92400e}.confirmed{background:#dcfce7;color:#166534}.cancelled{background:#fee2e2;color:#991b1b}
    ...
    <div class="page">
        ...
        <div class="card">
            <table>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($batch->payrollPeriod)
                                <a href="{{ route('payroll-periods.show', $batch->payrollPeriod) }}" style="color:#4c3b91;font-weight:900;text-decoration:none">
                        ...
                        <td>
                            @if($batch->missing_bank_data_count > 0)
                                <span class="warn">{{ $batch->missing_bank_data_count }}</span>
                            @else
                                0
                            @endif
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="9" class="empty">لا توجد دفعات تحويل حتى الآن.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
