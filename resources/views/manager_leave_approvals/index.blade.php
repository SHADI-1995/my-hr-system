...
@section('title', 'موافقات المدير المباشر')
@section('page-title', 'موافقات المدير المباشر')
...
@section('content')
    ...
    <div class="manager-approval-page">
        ...
        <div class="approval-hero">
            <div>
                <h1>موافقات المدير المباشر</h1>
                <p>هنا تظهر طلبات الإجازة الخاصة بالموظفين المرتبطين بك كمدير مباشر.</p>
            </div>
        ...
        <div class="filter-card">
            <form method="GET" action="{{ route('manager-leave-approvals.index') }}">
                ...
                <div class="filter-actions">
                    ...
                    <a href="{{ route('manager-leave-approvals.index') }}" class="approval-btn muted">مسح</a>
                </div>
            </form>
        </div>
        ...
        <div class="table-card">
            <table class="approval-table">
                ...
                <tr>
                    ...
                    <th style="width:17%;">نوع الإجازة</th>
                    <th style="width:17%;">الفترة</th>
                    <th style="width:70px;">الأيام</th>
                    <th style="width:17%;">الحالة</th>
                        ...
                        <td>
                            @if($leaveRequest->workflow_status === 'pending_manager')
                                <div class="row-actions">
                                    <form method="POST" action="{{ route('manager-leave-approvals.approve', $leaveRequest->id) }}">
                                        @csrf
                                        <button type="submit" class="approval-btn success" onclick="return confirm('تأكيد الموافقة على الطلب؟')">
                                            موافقة
                                        </button>
                                    </form>

                                    <button type="button" class="approval-btn danger" onclick="toggleRejectBox('reject-box-{{ $leaveRequest->id }}')">
                                        رفض
                                    </button>
                                </div>

                                <div id="reject-box-{{ $leaveRequest->id }}" class="reject-box">
                                    <form method="POST" action="{{ route('manager-leave-approvals.reject', $leaveRequest->id) }}">
                                        @csrf
                                        <div class="form-group">
